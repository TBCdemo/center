...
const SchedulingAndGovernance = ({ session, goBack, goToMembers, supabase, utils, constants, StatCard }) => {
    ...
    // ★ 資料梳理：將 allow_concurrent 整合進同工設定中
        ...
        return dbData.members
            ...
            .map(m => {
                ...
                return {
                    ...
                    allow_concurrent: qs?.allow_concurrent ?? m.allow_concurrent ?? false, // 讀取兼任設定
    ...
    const recommendations = useMemo(() => {
        ...
        let filtered = effectiveMembers.filter(m => {
            ...
            // ★ 兼任安全防護：如果對方設定不兼任，且當天已經有班，就不能作為推薦人選
            if (!m.allow_concurrent && mShiftsToday.length >= 1) return false;
