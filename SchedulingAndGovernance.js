            ...
            <div className="h-full overflow-y-auto custom-scrollbar p-6 lg:p-8 animate-fade-in pb-20 bg-slate-50">
                <div className="flex flex-col xl:flex-row gap-6 mb-8">
                    ...
                    <div className="flex-1 flex flex-col sm:flex-row gap-4 overflow-hidden">
                        <div className="bg-white p-5 rounded-xl shadow-soft border border-slate-100 flex-1 flex flex-col min-h-[160px] transform hover:rotate-x-[2deg] hover:rotate-y-[-4deg] transition-transform duration-300">
                            <h4 className="text-[13px] font-extrabold text-slate-500 mb-3 flex items-center gap-1.5"><BarChart3 size={14} className="text-sky-500"/> 服事天數分布圖</h4>
                        ...
                        <div className="bg-white p-5 rounded-xl shadow-soft border border-slate-100 flex-1 flex flex-col min-h-[160px] transform hover:rotate-x-[2deg] hover:rotate-y-[4deg] transition-transform duration-300">
                            <h4 className="text-[13px] font-extrabold text-slate-500 mb-3 flex items-center gap-1.5"><BarChart3 size={14} className="text-emerald-500"/> 服事次數分布圖</h4>
                            <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2.5 pr-2">
                ...
                <div className="bg-white rounded-xl shadow-soft border border-slate-100 overflow-hidden mb-8 flex flex-col max-h-[600px]">
                    <div className="p-6 lg:px-8 border-b border-slate-100 bg-slate-50 flex flex-col sm:flex-row sm:items-center justify-between gap-4 sticky top-0">
                        <h3 className="text-lg font-extrabold text-slate-900 flex items-center gap-2"><LayoutList className="text-indigo-600" size={20} /> 同工排班分析表</h3>
                        <div className="relative w-full sm:w-64">
                            <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                            <input type="text" placeholder="搜尋姓名..." value={analysisSearchTerm} onChange={e => setAnalysisSearchTerm(e.target.value)} className="w-full bg-white border border-slate-200 rounded-lg pl-9 pr-8 py-2 text-sm font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all shadow-sm" />
                    ...
                    <div className="overflow-x-auto custom-scrollbar flex-1">
                        <table className="w-full text-left border-collapse min-w-[800px]">
                            <thead className="sticky top-0 bg-white/95 backdrop-blur shadow-sm z-10">
                                <tr>
                                    <th onClick={() => requestSort('name')} className="py-4 px-4 font-bold text-slate-500 text-sm whitespace-nowrap pl-8 border-b border-slate-100 cursor-pointer hover:bg-slate-50 select-none">姓名 <ArrowUpDown size={14} className="inline ml-1 opacity-40"/></th>
                                    <th onClick={() => requestSort('totalService')} className="py-4 px-4 font-bold text-slate-500 text-sm whitespace-nowrap border-b border-slate-100 text-center cursor-pointer hover:bg-slate-50">服事次數 <ArrowUpDown size={14} className="inline ml-1 opacity-40"/></th>
                                    <th onClick={() => requestSort('attendance')} className="py-4 px-4 font-bold text-slate-500 text-sm whitespace-nowrap border-b border-slate-100 text-center cursor-pointer hover:bg-slate-50">出席天數 <ArrowUpDown size={14} className="inline ml-1 opacity-40"/></th>
                                    {['司會', '執事輪值', '接待', '收奉獻', '主餐', 'PPT', '新朋友關懷'].map(role => (
                                        <th key={role} onClick={() => requestSort(role)} className="py-4 px-4 font-bold text-slate-500 text-sm whitespace-nowrap border-b border-slate-100 text-center cursor-pointer hover:bg-slate-50">{role} <ArrowUpDown size={14} className="inline ml-1 opacity-40"/></th>
                                    ))}
                                </tr>
                            </thead>
                            <tbody>
                                {sortedDashboardData.map((d, i) => (
                                    <tr key={i} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                                        <td className="py-3 px-4 pl-8 whitespace-nowrap"><p className="font-extrabold text-slate-900 text-base">{d.name}</p></td>
                                        <td className="py-3 px-4 text-center font-extrabold text-indigo-600 bg-indigo-50/30">{d.totalService}</td>
                                        <td className="py-3 px-4 text-center font-bold text-slate-600">{d.attendance}</td>
                                        {['司會', '執事輪值', '接待', '收奉獻', '主餐', 'PPT', '新朋友關懷'].map(role => (
                                            <td key={role} className="py-3 px-4 text-center font-medium text-slate-400">{d.roles[role] || '-'}</td>
                                        ))}
                                    </tr>
                                ))}
                            </tbody>
                ...
                <div className="bg-white p-8 lg:p-10 rounded-xl shadow-soft border border-slate-100">
                    <h3 className="text-xl font-extrabold text-slate-900 mb-8">🛠️ 本季崗位需求 <span className="text-sm text-slate-500 font-bold ml-2">(合計：{dashboardStats.totalServices} 次)</span></h3>
            ...
            <div className="flex-1 flex items-center justify-center bg-slate-50 p-6 animate-fade-in overflow-y-auto">
                <div className="w-full max-w-xl bg-white p-10 lg:p-12 rounded-2xl shadow-soft border border-slate-100 relative">
                    <div className="flex bg-slate-100 p-1.5 rounded-xl mb-8">
                        <button onClick={() => setAppMode('schedule')} className={`flex-1 py-3.5 flex items-center justify-center gap-2 text-sm font-bold rounded-lg transition-all ${appMode === 'schedule' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-200/50'}`}><Play size={18} /> 預排作業</button>
                        <button onClick={() => setAppMode('query')} className={`flex-1 py-3.5 flex items-center justify-center gap-2 text-sm font-bold rounded-lg transition-all ${appMode === 'query' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-200/50'}`}><Search size={18} /> 編輯班表</button>
                    </div>
                    {appMode === 'schedule' ? (
                        <div className="animate-fade-in">
                            <div className="grid grid-cols-2 gap-6 mb-10">
                                <div className="space-y-2">
                                    <label className="text-xs font-bold text-slate-500 ml-2">年份</label>
                                    <input type="number" value={year} onChange={e => setYear(parseInt(e.target.value))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-6 py-4 font-medium text-slate-900 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all" />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-xs font-bold text-slate-500 ml-2">季度</label>
                                    <select value={quarter} onChange={e => setQuarter(parseInt(e.target.value))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-6 py-4 font-medium text-slate-900 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all">
                            ...
                            <button onClick={runAutoSchedule} disabled={isLoading} className="w-full bg-slate-900 hover:bg-slate-800 disabled:bg-slate-300 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-3 transition-all active:scale-95 shadow-button hover:-translate-y-0.5">
                        ...
                        <div className="animate-fade-in">
                            <div className="grid grid-cols-2 gap-6 mb-10">
                                <div className="space-y-2">
                                    <label className="text-xs font-bold text-slate-500 ml-2">年份</label>
                                    <input type="number" value={queryYear} onChange={e => setQueryYear(parseInt(e.target.value))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-6 py-4 font-medium text-slate-900 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all" />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-xs font-bold text-slate-500 ml-2">季度</label>
                                    <select value={queryQuarter} onChange={e => setQueryQuarter(parseInt(e.target.value))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-6 py-4 font-medium text-slate-900 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all">
                                        <option value={1}>Q1 (1-3月)</option><option value={2}>Q2 (4-6月)</option><option value={3}>Q3 (7-9月)</option><option value={4}>Q4 (10-12月)</option>
                                    </select>
                                </div>
                            </div>
                            <button onClick={runQuerySchedule} disabled={isLoading} className="w-full bg-gradient-to-r from-indigo-600 to-violet-600 hover:opacity-95 disabled:from-indigo-300 disabled:to-violet-300 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-3 transition-all active:scale-95 shadow-button hover:-translate-y-0.5">
            ...
            <div className="h-full flex flex-col bg-white animate-fade-in overflow-hidden shadow-soft">
                <div className="bg-slate-900 p-6 pb-8 shrink-0 relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-4 relative z-10">
                        <div className="w-14 h-14 rounded-full bg-gradient-to-br from-indigo-400 to-violet-500 flex items-center justify-center text-white text-xl font-extrabold shadow-glow">{stats.name.charAt(0)}</div>
                        <div className="text-white">
                            <h2 className="text-xl font-extrabold">{stats.name}</h2>
                            <p className="text-slate-400 text-xs font-medium flex items-center gap-2 mt-1">{stats.group ? <span className="bg-white/10 px-2 py-0.5 rounded text-[10px]">{stats.group}</span> : '一般同工'}<span>個人關懷儀表板</span></p>
                        </div>
                    </div>
                </div>
                <div className="flex-1 overflow-y-auto custom-scrollbar p-6 bg-slate-50 space-y-6 -mt-4 rounded-t-2xl relative z-10 border-t border-slate-200">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 text-center hover:shadow-soft transition-shadow">
                            <p className="text-[11px] font-semibold text-slate-400 mb-1">本季服事</p><p className="text-2xl font-extrabold text-slate-900">{stats.totalService} <span className="text-xs font-semibold text-slate-400">次</span></p>
                            <p className={`text-[10px] font-bold mt-1.5 px-1.5 py-0.5 rounded-md inline-block ${diffFromAvg > 0 ? 'bg-rose-50 text-rose-600' : (diffFromAvg < 0 ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-500')}`}>{diffFromAvg > 0 ? `+${diffFromAvg} 高於平均` : (diffFromAvg < 0 ? `${diffFromAvg} 低於平均` : '與平均持平')}</p>
                        </div>
                        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 text-center hover:shadow-soft transition-shadow">
                            <p className="text-[11px] font-semibold text-slate-400 mb-1">出席天數</p><p className="text-2xl font-extrabold text-slate-900">{stats.attendance} <span className="text-xs font-semibold text-slate-400">天</span></p>
                            <p className="text-[10px] font-semibold text-slate-400 mt-1.5">單日最高 {(stats.totalService / Math.max(1, stats.attendance)).toFixed(1)} 次</p>
                        </div>
                    </div>
                    <div>
                        <h3 className="text-xs font-extrabold text-slate-900 mb-3 flex items-center gap-1.5"><Lightbulb size={14} className="text-amber-500"/> AI 關懷建議</h3>
                        <div className="space-y-2.5">
                            {adviceList.map((adv, idx) => (
                                <div key={idx} className={`${adv.bg} border border-white shadow-sm p-3 rounded-xl flex gap-3 items-start`}>
                                    ...
                                    <div><p className={`text-[13px] font-extrabold ${adv.color} mb-0.5`}>{adv.title}</p><p className="text-[11px] font-medium text-slate-600 leading-relaxed">{adv.desc}</p></div>
                                </div>
                            ))}
                        </div>
                    </div>
                    <div>
                        <h3 className="text-xs font-extrabold text-slate-900 mb-3 flex items-center gap-1.5"><BarChart3 size={14} className="text-indigo-500"/> 崗位參與分佈</h3>
                        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 space-y-3">
                            {Object.entries(stats.roles).filter(([_, count]) => count > 0).sort((a,b)=>b[1]-a[1]).map(([role, count]) => (
                                <div key={role}>
                                    <div className="flex justify-between text-[11px] font-semibold mb-1"><span className="text-slate-700">{role}</span><span className="text-indigo-600">{count} 次 <span className="text-slate-400 font-normal">({Math.round(count/stats.totalService*100)}%)</span></span></div>
        ...
        return (
            <div className="w-full lg:w-[400px] xl:w-[450px] shrink-0 bg-white border-l border-slate-200 overflow-hidden h-full flex flex-col shadow-soft z-20 animate-panel-right relative">
                <div className="bg-slate-900 p-6 rounded-b-[1.5rem] shadow-sm border-b border-slate-800 relative z-20 shrink-0 overflow-hidden">
                    ...
                    <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 mt-1 relative z-10">幫此服事找合適人選：</h3>
                    <div className="flex items-start gap-3 relative z-10">
                        ...
                        <div className="flex flex-col gap-1.5">
                            <p className="text-xl font-extrabold text-white">{activeSlot.service_date}</p>
                            <div className="flex items-center gap-2 flex-wrap">
                                <span className={`px-2 py-0.5 rounded-md text-sm font-extrabold ${activeSlot.is_empty ? 'bg-rose-500 text-white animate-pulse shadow-[0_0_15px_rgba(244,63,94,0.5)]' : 'bg-gradient-to-r from-indigo-500 to-violet-500 text-white'}`}>{activeSlot._memberName}</span>
                            </div>
                            <div className="flex items-center gap-1.5 text-xs font-medium text-slate-400 flex-wrap mt-1">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar px-6 pb-6 pt-2 space-y-4">
                    <div className="flex items-center justify-between px-1">
                        <h3 className="font-extrabold text-slate-900 flex items-center gap-2 text-sm"><UserCheck className="text-emerald-500" size={16}/> 推薦人選 ({finalRecommendations.length})</h3>
                        <span className="text-[10px] bg-slate-100 text-slate-500 px-2 py-1 rounded-md font-bold">依本季次數排序</span>
                    </div>
                    {finalRecommendations.length > 0 ? (
                        finalRecommendations.map((c, idx) => (
                            <div key={c.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm transition-all hover:shadow-soft hover:-translate-y-0.5">
                                <div className="flex items-center gap-3">
                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center font-extrabold text-white shadow-sm text-xs ${idx < 3 && !searchTerm ? 'bg-gradient-to-br from-emerald-400 to-emerald-600' : 'bg-slate-300'}`}>{idx + 1}</div>
                                    <div>
                                        <div className="flex items-center gap-2"><p className="text-[15px] font-extrabold text-slate-900">{c.name}</p></div>
                                        <div className="flex items-center gap-1 text-[11px] font-medium text-slate-500 mt-1 flex-wrap">
                                ...
                                <div className="mt-3 pt-3 border-t border-slate-100">
                                    {c.swapOptions && c.swapOptions.length > 0 ? (
                                        <div className="space-y-2.5">
                                            <p className="text-[10px] font-semibold text-slate-400 flex items-center gap-1"><RefreshCw size={10} /> 雙方可以互換的班次：</p>
                                            <div className="flex flex-col gap-2">
                                                {c.swapOptions.map((swap, i) => (
                                                    <div key={i} className="flex items-center justify-between bg-indigo-50/50 text-indigo-800 px-2.5 py-1.5 rounded-lg border border-indigo-100">
                                                        ...
                                                        <button onClick={() => requestSwap(c, swap)} className="flex items-center gap-1 bg-white text-indigo-600 hover:bg-indigo-600 hover:text-white border border-indigo-200 px-2 py-1 rounded-md text-[10px] font-bold transition-colors shadow-sm"><RefreshCw size={10} /> 換班</button>
                                                    </div>
                                                ))}
                                            </div>
                                            <div className="flex items-center justify-between bg-slate-50 px-2.5 py-1.5 rounded-lg border border-slate-200 mt-1">
                                                ...
                                                <button onClick={() => requestSubstitute(c)} className="flex items-center gap-1 bg-white text-orange-600 hover:bg-orange-500 hover:text-white border border-orange-200 px-2 py-1 rounded-md text-[10px] font-bold transition-colors shadow-sm"><HandHeart size={10} /> 替補</button>
                                            </div>
                                        </div>
                                    ) : (
                        ...
                        <div className="p-8 text-center text-slate-400 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                            ...
                            <p className="font-semibold text-sm">{searchTerm ? '找不到符合搜尋條件的人選' : '找不到合適人選'}</p>
    ...
    return (
        <div className="flex h-screen w-full bg-slate-50 overflow-hidden select-none relative">
            ...
            <div className="w-64 bg-slate-900 flex flex-col justify-between shrink-0 border-r border-slate-800 z-30">
                <div className="flex flex-col">
                    ...
                    <div className="p-6 border-b border-slate-800 flex items-center gap-3 relative overflow-hidden">
                        ...
                        <span className="text-white font-extrabold text-base tracking-wider relative z-10">TBC Serve Manager</span>
                    </div>
                    ...
                    <nav className="p-4 space-y-1.5">
                        ...
                        <button onClick={goToMembers} className="w-full flex items-center gap-3 px-4 py-3 text-slate-400 hover:text-white hover:bg-slate-800/60 rounded-xl font-medium text-sm transition-all text-left group">
                            <Users size={18} className="text-slate-400 group-hover:text-violet-400 transition-colors" />
                            <span>同工資料中心</span>
                        </button>
                        <div className="flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-xl font-bold text-sm shadow-button">
                            <Calendar size={18} />
                            <span>排班作業中心</span>
                        </div>
                    </nav>
            ...
            <div className="flex-1 flex flex-col overflow-hidden bg-slate-50 relative">
                        ...
                        {schedulingPhase === 'editor' && (
                            <>
                                ...
                                <div className="flex gap-3 mt-2 pt-2 border-t border-slate-100 flex-wrap">
                                    <p className="text-rose-600 text-[10px] font-extrabold flex items-center gap-1.5 bg-rose-50 px-2 py-1 rounded"><span className="w-2 h-2 rounded-full bg-rose-500"></span> 紅色：崗位兼任</p>
                                    <p className="text-sky-600 text-[10px] font-extrabold flex items-center gap-1.5 bg-sky-50 px-2 py-1 rounded"><span className="w-2 h-2 rounded-full bg-sky-500"></span> 藍色：群組落單</p>
                                    {appMode === 'schedule' && <p className="text-orange-600 text-[10px] font-extrabold flex items-center gap-1.5 bg-orange-50 px-2 py-1 rounded"><span className="w-2 h-2 rounded-full bg-orange-500"></span> 橘色：落單自動替換</p>}
                                </div>
                            </>
                        )}
                    </div>
                            ...
                            <div className="flex items-center gap-3 flex-wrap justify-end">
                                <div className="flex bg-slate-50 p-1.5 rounded-lg w-full md:w-auto overflow-x-auto custom-scrollbar border border-slate-200">
                                    {['第一堂', '第二堂', '📊 數據分析'].map(tab => (
                                        <button key={tab} onClick={() => { setActiveSessionTab(tab); if(tab === '📊 數據分析') setActiveSlot(null); }} className={`px-5 py-2 rounded-md text-sm font-bold transition-all duration-200 whitespace-nowrap ${activeSessionTab === tab ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-200/50'}`}>{tab}</button>
                                    ))}
                                    {appMode === 'schedule' && (
                                        <>
                                            <div className="w-px h-6 bg-slate-200 mx-2 self-center"></div>
                                            <button onClick={runAutoSchedule} disabled={isLoading} className="px-4 py-2 rounded-md text-sm font-bold transition-all duration-200 whitespace-nowrap text-indigo-600 hover:bg-white hover:shadow-sm flex items-center gap-1.5"><RefreshCw size={16} className={isLoading ? "animate-spin" : ""} /> 重新排班</button>
                                        </>
                                    )}
                                </div>
                                <div className="flex bg-slate-50 p-1.5 rounded-lg w-full md:w-auto overflow-x-auto custom-scrollbar border border-slate-200">
                                    <button onClick={exportToCSV} className="px-4 py-2 rounded-md text-sm font-bold transition-all duration-200 whitespace-nowrap text-emerald-600 hover:bg-white hover:shadow-sm flex items-center gap-1.5"><Download size={16} /> 匯出 CSV</button>
                                    <button onClick={handlePublishClick} disabled={isSaving} className="px-4 py-2 rounded-md text-sm font-bold transition-all duration-200 whitespace-nowrap bg-gradient-to-r from-indigo-600 to-violet-600 text-white shadow-button hover:-translate-y-0.5 flex items-center gap-1.5 disabled:from-indigo-400 disabled:to-violet-400">{isSaving ? <RefreshCw className="animate-spin" size={16} /> : <><Save size={16}/> 發布班表</>}</button>
                                </div>
                            </div>
                        </div>
                    )}
                ...
                {/* Modals & Toasts */}
                {errorMsg && <div className="absolute top-4 left-1/2 transform -translate-x-1/2 z-50 bg-red-50 text-red-600 px-6 py-3 rounded-lg flex items-center gap-3 font-bold border border-red-100 shadow-xl animate-bounce"><AlertCircle size={20} /> {errorMsg}</div>}
                {showSuccessToast && <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-[200] bg-emerald-50 text-emerald-600 px-8 py-5 rounded-2xl flex items-center gap-4 font-extrabold text-xl border-2 border-emerald-200 shadow-glow animate-pop"><CheckCircle2 size={32} className="text-emerald-500" /> 發布成功</div>}
                ...
                {confirmDialog.isOpen && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fade-in">
                        <div className="bg-white rounded-2xl p-6 md:p-8 max-w-md w-full shadow-hover-soft animate-pop border border-slate-100">
                            <h3 className="text-xl font-extrabold text-slate-900 mb-6 flex items-center gap-2">{confirmDialog.type === 'swap' ? <RefreshCw className="text-indigo-500" /> : <HandHeart className="text-orange-500" />}{confirmDialog.title}</h3>
                            <div className="mb-8 p-4 sm:p-5 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-between gap-2 sm:gap-4 shadow-inner">
                                <div className="flex-1 text-center break-words"><p className="text-[10px] font-semibold text-slate-400 mb-1.5">目前同工</p><p className="text-sm sm:text-base font-extrabold text-slate-900">{confirmDialog.currentName}</p><div className="text-[10px] sm:text-[11px] text-slate-500 mt-1 leading-snug"><p>{confirmDialog.currentDate}</p><p>{confirmDialog.currentRole}</p></div></div>
                                ...
                                <div className="flex-1 text-center break-words"><p className="text-[10px] font-semibold text-slate-400 mb-1.5">替換同工</p><p className={`text-sm sm:text-base font-extrabold ${confirmDialog.type === 'swap' ? 'text-indigo-600' : 'text-orange-600'}`}>{confirmDialog.newName}</p><div className="text-[10px] sm:text-[11px] text-slate-500 mt-1 leading-snug"><p>{confirmDialog.newDate}</p><p>{confirmDialog.newRole}</p></div></div>
                            </div>
                            <div className="flex gap-3">
                                <button onClick={() => setConfirmDialog({ ...confirmDialog, isOpen: false })} className="flex-1 py-3 px-4 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-bold rounded-lg transition-colors">取消</button>
                                <button onClick={confirmDialog.onConfirm} className={`flex-1 py-3 px-4 font-bold text-white rounded-lg transition-all duration-200 shadow-button hover:-translate-y-0.5 active:scale-95 ${confirmDialog.type === 'swap' ? 'bg-gradient-to-r from-indigo-600 to-violet-600' : 'bg-gradient-to-r from-orange-500 to-amber-500'}`}>確認執行</button>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {publishConfirmOpen && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fade-in">
                        <div className="bg-white rounded-2xl p-6 md:p-8 max-w-md w-full shadow-hover-soft animate-pop border border-slate-100">
                            <div className="flex items-center gap-3 mb-4 text-indigo-600"><AlertCircle size={28} /><h3 className="text-2xl font-extrabold text-slate-900">準備發布班表</h3></div>
                            <div className="mb-8 bg-slate-50 p-5 rounded-xl border border-slate-100">
                                <p className="text-slate-700 font-bold mb-3">溫馨小提醒</p><p className="text-slate-500 text-sm font-medium flex items-start gap-2"><Info size={16} className="text-emerald-500 shrink-0 mt-0.5" /><span>尚未匯出試算表檔案，請點擊「取消返回」，使用「匯出 CSV」功能，以利後續「服事表排版」。</span></p>
                            </div>
                            <div className="flex gap-3">
                                <button onClick={() => setPublishConfirmOpen(false)} className="flex-1 py-3 px-4 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-bold rounded-lg transition-colors">取消返回</button>
                                <button onClick={executePublish} className="flex-1 py-3 px-4 font-bold text-white bg-gradient-to-r from-indigo-600 to-violet-600 rounded-lg transition-all duration-200 shadow-button hover:-translate-y-0.5 active:scale-95 flex items-center justify-center gap-2"><Save size={18} /> 確認發布</button>
                            </div>
                        </div>
                    </div>
                )}
